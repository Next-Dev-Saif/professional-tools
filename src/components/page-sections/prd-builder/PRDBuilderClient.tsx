...
export function PRDBuilderClient() {
  ...
  useEffect(() => {
    registerPage(
      ...
      (data: any) => {
        ...
        if (Array.isArray(data.outOfScope)) setOutOfScope(data.outOfScope);
        if (Array.isArray(data.dependencies)) setDependencies(data.dependencies);
      }
    );
  ...
  }, [viewMode, projectName, objective, outOfScope, dependencies]);
      ...
      <div className="w-full md:w-[320px] lg:w-[400px] xl:w-[460px] h-full flex flex-col border-b md:border-b-0 md:border-r border-border/50 bg-surface/30 backdrop-blur-md relative z-10">
        ...
        <div className="flex-none p-6 border-b border-border/50 bg-background/80 sticky top-0 z-20">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight">PRD Builder</h1>
              <p className="text-sm text-foreground/60">Product Requirements Document</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar pb-32">
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wide text-foreground/70 mb-1.5">Project Name</label>
            <input 
              type="text" 
              value={projectName}
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wide text-foreground/70 mb-1.5">Objective</label>
            <textarea 
              value={objective}
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-semibold uppercase tracking-wide text-foreground/70">Out of Scope</h3>
            {outOfScope.map((item, i) => (
              <input 
                ...
                value={item}
                onChange={(e) => {
                  const newArr = [...outOfScope];
                  newArr[i] = e.target.value;
                  setOutOfScope(newArr);
                }}
            ...
            <button onClick={() => setOutOfScope([...outOfScope, ''])} className="text-xs text-primary font-bold bg-primary/10 px-3 py-1.5 rounded-lg">+ Add Item</button>
          </div>

          <div className="h-px bg-border/50 w-full" />

          <div className="space-y-4">
            <h3 className="text-xs font-semibold uppercase tracking-wide text-foreground/70">Dependencies</h3>
            ...
            <button onClick={() => setDependencies([...dependencies, ''])} className="text-xs text-primary font-bold bg-primary/10 px-3 py-1.5 rounded-lg">+ Add Item</button>
          </div>
        </div>
      </div>
              ...
              >
                <div className="max-w-3xl mx-auto">
                  ...
                  <div className="space-y-10">
                    <section>
                      <h2 className="text-xl font-bold text-zinc-900 mb-4 border-b pb-2">Project Name</h2>
                      <div className="text-zinc-700 whitespace-pre-wrap">{projectName}</div>
                    </section>

                    <section>
                      <h2 className="text-xl font-bold text-zinc-900 mb-4 border-b pb-2">Objective</h2>
                      <div className="text-zinc-700 whitespace-pre-wrap">{objective}</div>
                    </section>

                    <section>
                      <h2 className="text-xl font-bold text-zinc-900 mb-4 border-b pb-2">Out of Scope</h2>
                      <ul className="list-disc pl-5 space-y-2 text-zinc-700">
                        {outOfScope.map((item, i) => item && <li key={i}>{item}</li>)}
                      </ul>
                    </section>

                    <section>
                      <h2 className="text-xl font-bold text-zinc-900 mb-4 border-b pb-2">Dependencies</h2>
                      <ul className="list-disc pl-5 space-y-2 text-zinc-700">
                        {dependencies.map((item, i) => item && <li key={i}>{item}</li>)}
                      </ul>
                    </section>
                  </div>
                </div>
